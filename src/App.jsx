    ...
    <div style={{ fontFamily: "'Noto Sans TC',sans-serif", background: "#f8fafc", minHeight: "100vh", paddingBottom: 50 }}>
                            ...
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                              ...
                              {isAdmin && (
                                <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
                                  ...
                                  {viewMode === "week" && <span style={{ fontSize: 10, color: "#fff", background:"#64748b", padding:"2px 4px", borderRadius:4 }}>{tech.name || "未知"}</span>}
                                </div>
                              )}
                            </div>
      ...
      <Modal open={modals.task} onClose={() => setModals({})} title={editing.id ? "編輯排程" : "新增排程"}>
        ...
        <button onClick={() => {
          ...
          addLog(`為 ${safeTechs.find(t=>t&&t.id===editing.techId)?.name || "未知"} ${editing.id ? '更新' : '排定'}了 ${theDate} 的行程`);
          setModals({});
      ...
      <Modal open={modals.logs} onClose={() => setModals({})} title="系統近期動態追蹤">
        ...
        <div style={{ maxHeight: 400, overflowY: "auto", paddingRight: 5 }}>
          {safeLogs.length > 0 ? safeLogs.map((l, i) => (
            <div key={l?.id || i} style={{ padding: "14px 12px", borderBottom: "1px solid #e2e8f0", fontSize: 14, display:"flex", gap:10 }}>
    ...
    if (this.state.hasError) {
      return (
        <div style={{ padding: "40px", textAlign: "center", fontFamily: "sans-serif" }}>
          <h2 style={{ color: "#ef4444" }}>⚠️ 系統畫面發生錯誤 (防白畫面機制啟動)</h2>
          <p>這通常是因為舊的資料格式不相容所導致的。請點擊下方按鈕重新載入系統：</p>
          ...
          <div style={{ background: "#f1f5f9", padding: "10px", borderRadius: "8px", color: "#334155", textAlign: "left", maxWidth: "800px", margin: "0 auto", overflowX: "auto" }}>
